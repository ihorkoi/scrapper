...
if (typeof GemiusHitcolConnector == "undefined") {
	GemiusHitcolConnector = function(is_gde) {
		...
		this.fpcap = {nsec: "", sec: ""};
		this.fpdbg = "";
		this.rudata = {rudata: "", ruid: "", refreshts: 0, prefix: "", type: "", refresh_interval: 10};
		...
		this.bfview = null;
		this.timeouts = {initto: null, cmpto: null, sto: null, fto: null, tfpto: null, tfplto: null, chto: null, ruidto: null, truidlto: null};
		this.init_called = 0;
		this.started = 0;
		this.visapi_s = "";
		this.visapi_h = "";
...
var check_same_origin = function(ref, page_location) {
	...
	return null;
		...
		this.sendhit = function(robj,nr,consent,lsadd) {
			...
			params += this.inner_params(consent);
			...
			try {
				...
				if (window.top === window.self) {
					...
					if (is_event("_xx_gemius_getruid_xx_/")) {
						...
						var clbk = function(ruid,status) {
							ruid = (typeof ruid === "string")?ruid:"";
							var msg = "_xx_gemius_putruid_xx_/" + ruid + "/" + status;
...
// cmp_utils.js 
		...
		this.inner_params = function(consent) {
			if (typeof encodeURIComponent != 'undefined') {
				...
				if (this.rudata.prefix!='') {
					inner += '|_ruid=' + (consent?this.rudata.ruid:'-NOCONSENT');
				}
				if (this.truid.ruid) {
					inner += '|_truid=' + (consent?this.truid.ruid:'-NOCONSENT');
				}
				if (this.lsdbg) {
...
// chints_params.js 
	...
	return params;
		...
		this.parse_rudata = function(rudata) {
			...
			this.rudata.ruid = rudata_splitted.length>0?rudata_splitted[0]:rudata;
			var rts = rudata_splitted.length>1?parseInt(rudata_splitted[1]):d;
			this.rudata.refreshts = (!rts||rts>d)?d:rts;
			this.rudata.type = rudata_splitted.length>2?rudata_splitted[2]:'';
			var rint = rudata_splitted.length>3?parseInt(rudata_splitted[3]):10;
		...
		this.setrucookie = function() {
			this.rudata.rudata = this.rudata.ruid + '|' + this.rudata.refreshts + '|' + this.rudata.type + '|' + this.rudata.refresh_interval;
			if (this.fpdata_sec_state!='unknown' && this.fpcdomain!='') {
				gemius_cookie.set(this.rucookie_name, this.rudata.rudata, this.rucookie_expire, this.fpcdomain, this.fpdata_sec_state, false);
			}
		}
		this.getrudataencoded = function() {
			if (typeof encodeURIComponent != 'undefined') {
				return encodeURIComponent(this.rudata.rudata);
			} else {
		...
		this.set_rudata = function(rudata) {
			...
			if (typeof localStorage != 'undefined' && localStorage != null) {
				if (this.rudata.ruid!='' && this.rudata.ruid[0]!='-') {
					localStorage.gemius_ruid = this.rudata.ruid;
				} else {
		...
		this.call_rudata_callbacks = function(ruid, status) {
			...
			for (var id in this.rudata_frame_callbacks) {
				if (this.rudata_frame_callbacks.hasOwnProperty(id)) {
					try {
						this.rudata_frame_callbacks[id](ruid, status);
					} catch (e) {}
				}
			}
		};
			...
			if (this.init_called && this.waiting_for_consent==0) {
				if (this.has_consent) {
					if (window.top !== window.self && this.ref.frametype != 2) {
						...
						} else if (this.waiting_for_rudata==0 && this.rudata.ruid!='') {
		...
		this.try_and_load_rudata = function() {
			if ((window.top === window.self || this.ref.frametype == 2) && this.waiting_for_consent==0 && this.has_consent && this.waiting_for_fpdata==0 && this.fpdata!='' && this.fpdata[0]!='-') {
				...
				if (this.rudata.refreshts<d) {
					...
					if (this.rudata.rudata != '' && this.rudata.rudata[0] != '-' && this.rudata.ruid!='') {
...
})('gemius_hcconn', {'referrer_override':'referrer_override', 'get_ruid':'get_ruid'});
